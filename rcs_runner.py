...
def run_rcs(templates_raw: list[dict], log_path: str = "rcs_submission_log.jsonl", client: str = "bajaj") -> None:
    """Submit each RCS DLT template, log the attempt."""
    submissions = load_rcs_from_list(templates_raw)
    print(f"Loaded {len(submissions)} RCS template(s) for {client} to submit.")
...
def run_rcs_file(file_path: str, log_path: str = "rcs_submission_log.jsonl", client: str = "bajaj") -> None:
    ...
    print(f"Loaded {len(submissions)} RCS template(s) for {client} from {file_path} to submit.")
