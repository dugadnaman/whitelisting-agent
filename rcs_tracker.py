...
Directly persists to database (job_tasks table with channel='rcs') as the single source of truth.
"""
...
def log_rcs_result(result: RcsSubmissionResult, log_path: str = "rcs_submission_log.jsonl") -> None:
    """Store RCS submission result directly into database (job_tasks) as single source of truth."""
    db.init_database()
    ...
    job_id = "job_direct_submissions_rcs"
    ...
    try:
        with db.get_db() as conn:
            conn.execute(
                ...
                VALUES (?, ?, 'rcs', 'direct_submission_rcs', 1, 'COMPLETED', ?, ?)
        ...
        logger.error("Failed to store RCS submission result in database: %s", exc)

    # File fallback only if a non-default custom file path was passed in unit tests
    if log_path != "rcs_submission_log.jsonl" and not log_path.endswith("rcs_submission_log.jsonl"):
        try:
...
def load_rcs_log(log_path: str = "rcs_submission_log.jsonl") -> list[dict]:
    """Read back all RCS submission results from the database."""
    if log_path != "rcs_submission_log.jsonl" and not log_path.endswith("rcs_submission_log.jsonl"):
        p = Path(log_path)
        if p.exists():
            entries = []
    ...
    try:
        with db.get_db() as conn:
            cur = conn.execute("SELECT * FROM job_tasks WHERE channel = 'rcs' ORDER BY created_at ASC")
            for r in cur.fetchall():
                entries.append(_row_to_entry_dict(dict(r)))
    except Exception as exc:
        logger.error("Failed to load RCS submission log from database: %s", exc)
    return entries


def update_rcs_result(source_ref: str, updates: dict, log_path: str = "rcs_submission_log.jsonl") -> bool:
    """Patch one RCS entry in the database (matched by source_ref or template_name)."""
...
def update_rcs_results(updates_by_ref: dict, log_path: str = "rcs_submission_log.jsonl") -> int:
    ...
    try:
            ...
            for ref, upd in updates_by_ref.items():
                ...
                cur = conn.execute(
                    ...
                    UPDATE job_tasks SET
                        ...
                        updated_at = ?
                    WHERE channel = 'rcs' AND (template_name = ? OR source_ref = ?)
                """,
        ...
        logger.error("Failed to update RCS results in database: %s", exc)
